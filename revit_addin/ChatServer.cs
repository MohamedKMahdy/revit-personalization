...
/// <summary>
/// Locates and launches the Python chatbot server (chatbot/chat_server.py) and
/// probes whether it is serving. Shared by OpenAssistantCommand and AssistantPane.
///
...
internal static class ChatServer
{
    ...
    /// re-checked under a lock, so it is a no-op when already serving.
    /// Never call on the Revit UI thread: it can block up to <paramref name="waitSeconds"/>.
    /// </summary>
    public static bool EnsureRunning(int waitSeconds = 12)
    {
        ...
        lock (_launchGate)
        {
            // Another thread may have started (and bound) the server while we waited.
            if (IsUp()) return true;
            ...
            // Prefer pythonw.exe (no console window flashing) when available.
            var launcher = PreferPythonw(pythonExe);
